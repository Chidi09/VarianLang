...
| Progressive form actions | SvelteKit actions/enhance | Native form fallback plus optional Lumen event transport | **done** — native named controls/action/method, zero-JS mode, typed server validation, structured field/form results, exact opt-in event action, lifecycle tests |
...
| Data loading and invalidation | SvelteKit load/dependencies | Ordered layout/page loaders plus explicit stable dependency keys | **done** — outer-to-inner inherited data, deduplicated dependency aggregation, keyed resources and mutation invalidation, loader contract tests |
